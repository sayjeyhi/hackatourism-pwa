...
const MusicPlayer = props => {
  ...
  return (
    <StyledMusicPlayer>
      ...
      />
      <StyledTimer>
        <p>{(duration - (Math.floor(duration / 60)) * 60).toFixed(0)} : {Math.floor(duration / 60)}</p>
        <p>{(elapsed - (Math.floor(elapsed / 60)) * 60).toFixed(0)} : {Math.floor(elapsed / 60)}</p>
      </StyledTimer>
      <input type="range" value={played} onChange={handleSeekChange} onMouseUp={handleSeekMouseUp} min={0} max={1} step="any"/>
      <div>
